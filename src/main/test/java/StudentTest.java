...
public class StudentTest extends BaseSqlSession {
     ...
     */
    @Test
    public void PrimaryKeyAutoIncreTest() {
        Student student = new Student(null, "令狐冲", true, 'A', "四川省");
        StudentMapper mapper = sqlSession.getMapper(StudentMapper.class);
        int i = mapper.saveStudent(student);
        sqlSession.commit();
     ...
     */
    @Test
    public void PrimaryKeyNoAutoIncreTest() {
        Student student = new Student(null, "令狐冲", true, 'A', "四川省");
        StudentMapper mapper = sqlSession.getMapper(StudentMapper.class);
        int i = mapper.saveStudent1(student);
        sqlSession.commit();
        System.out.println(i);
    }
}